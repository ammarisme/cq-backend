...
- 🎯 Entry points to execute your program using `python -m <cq_backend>` or `$ cq_backend` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/ammarisme/cq-backend/branch/main/graph/badge.svg?token=cq-backend_token_here)](https://codecov.io/gh/ammarisme/cq-backend)
[![CI](https://github.com/ammarisme/cq-backend/actions/workflows/main.yml/badge.svg)](https://github.com/ammarisme/cq-backend/actions/workflows/main.yml)

Awesome cq_backend created by ammarisme

## Install

from source
```bash
git clone https://github.com/ammarisme/cq-backend cq_backend
cd cq_backend
make install
```
...
```bash
pip install cq_backend
```

## Executing

```bash
$ cq_backend run --port 8080
```

or

```bash
python -m cq_backend run --port 8080
```

or

```bash
$ uvicorn cq_backend:app
```
...
```bash
❯ cq_backend --help
Usage: cq_backend [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ cq_backend create-user --help
Usage: cq_backend create-user [OPTIONS] USERNAME PASSWORD
...
```bash
cq_backend create-user admin admin --superuser
```
...
Run with `cq_backend run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `cq_backend create-user`.
...
-----------------------------------------------------
cq_backend/__init__.py              4      0   100%
cq_backend/app.py                  16      1    94%
cq_backend/cli.py                  21      0   100%
cq_backend/config.py                5      0   100%
cq_backend/db.py                   10      0   100%
cq_backend/models/__init__.py       0      0   100%
cq_backend/models/content.py       47      1    98%
cq_backend/routes/__init__.py      11      0   100%
cq_backend/routes/content.py       52     25    52%
cq_backend/routes/security.py      15      1    93%
cq_backend/routes/user.py          52     26    50%
cq_backend/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in cq_backend/default.toml.
